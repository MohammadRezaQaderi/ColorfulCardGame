...
def Cost_of_state(state , Num_color, Max_num):
    cost = 0
    colors = []
    for i in range(len(state)):
        for j in range(len(state[i])):
            num , color = split_card(state[i][j])
            if(int(num) == int(Max_num)):
                colors.append(color)
                for l in range(int(Max_num)-1):  
                    num1 = int(Max_num) - int(l) - int(1)
                    card = concat_card(num1 , color)
                    for k in range(len(state[i])):
                        if(card == state[i][k] and k > j):
                            break
                    else:
                        cost +=1
                break

    cost=cost+int((int(Num_color) - len(colors)) * int( Max_num))
    return cost       
...
def Child_Node(frontier , explored , Num_col , Max_num , Num_color):
        ...
        if(len(state[0][i]) == 0 and flag == 0):
            ...
            for j in range(len(state[0])):
                ...
                if(len(Child[0][j]) == 0 and flag == 0):
                    ...
                    Child[1] =int(Cost_of_state(Child[0] , Num_color , Max_num))
...
def BFS(initial_state , Num_col , Max_num , Num_color):
    state = deepcopy(initial_state)
    if(goal(state ,Num_col)):
        return state
    explored = []
    frontier = []
    frontier.append([state ,Cost_of_state(state , Num_color , Max_num)])
    flag = 0
    count = 5
    while True:
        if not frontier:
            return False
        frontier , flag = Child_Node(frontier , explored , Num_col , Max_num , Num_color)
        if(flag == 1):
